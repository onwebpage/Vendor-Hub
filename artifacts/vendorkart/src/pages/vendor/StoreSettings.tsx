...
import { z } from "zod";
import { Store, Loader2, Save, AlertCircle, MapPin, Phone, Wallet, Lock, Link2, Copy, ExternalLink, Upload, QrCode, ShieldCheck, ShieldOff, X, Eye, EyeOff } from "lucide-react";
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-muted/20">
        ...
        {twoFactorEnabled ? (
          <Button type="button" variant="outline" size="sm" className="rounded-xl text-xs h-9 gap-2 border-red-200 text-red-600 hover:bg-red-50" onClick={() => setShowDisableModal(true)} disabled={loading}>
            {loading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <ShieldOff className="w-3.5 h-3.5" />}
            Disable
          </Button>
        ) : (
          <Button type="button" size="sm" className="rounded-xl text-xs h-9 gap-2" onClick={handleEnable} disabled={loading}>
          ...
          <div className="relative bg-card rounded-3xl border border-border shadow-2xl w-full max-w-md p-8">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <Button type="button" onClick={handleDisable} disabled={loading} className="flex-1 rounded-xl h-11 gap-2 bg-red-600 hover:bg-red-700 text-white">
